...
class Atlas_Model_BottleTypesMapper {
    ...
    public function getDbTable() {
        ...
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Atlas_Model_DbTable_BottleTypes');
        }
    ...
    public function save(Atlas_Model_BottleTypes $b_type) {
        // push the data into an array
        $data = $b_type->toArray();

        // if the row in the db doesnt exist create the row
        // otherwise update the existing row
        if (NULL === ($bottle_type_id = $b_type->getBottle_type_id()) || $bottle_type_id == 0) {
            unset($data['bottle_type_id']);
            $bottle_type_id = $this->getDbTable()->insert($data);
            return $bottle_type_id;
        } else {
            $this->getDbTable()->update($data, array('bottle_type_id = ?' => $bottle_type_id));
            return $bottle_type_id;
        }
    }
    ...
    public function remove($bottle_type_id) {
        $this->getDbTable()->delete("bottle_type_id='$bottle_type_id'");
    }

#end remove() function
    // find a row in the database based on the primary key and set the values
    // in the db object given by the user
    public function find($bottle_type_id) {
        $b_type = new Atlas_Model_BottleTypes();

        // attempt to locate the row in the database
        // if it doesn't exist throw an exception
        $result = $this->getDbTable()->find($bottle_type_id);
        if (0 == count($result)) {
    ...
    public function fetchAll() {
        ...
        foreach ($resultSet as $row) {
            $entry = new Atlas_Model_BottleTypes();
    ...
    public function selectAll() {
        ...
        $select->setIntegrityCheck(false)
                ->from(array("bt" => "bottle_types"), array("bt.bottle_type_id", "bt.bottle_type"))
                ->order("bt.bottle_type ASC");
    ...
    public function buildAll() {
        $select = $this->getDbTable()->select();
        $select->setIntegrityCheck(false)
                ->from(array("bs" => "bottle_types"), array("bs.*"))
                ->order("bs.bottle_type ASC");
    ...
    public function buildIdByName($name) {
        $select = $this->getDbTable()->select();
        $select->setIntegrityCheck(false)
                ->from(array("bt" => "bottle_types"), array("bt.bottle_type_id"))
                ->where("bt.bottle_type LIKE '%" . $name . "%'");

        $result = $select->query()->fetchAll();
        return $result[0]['bottle_type_id'];
    }
    ...
    public function checkLabel($entry_id, $label) {
        $select = $this->getDbTable()->select();
        $select->from(array("t" => "bottle_types"), array('COUNT(*) AS count'))
                ->where('LOWER(t.bottle_type) = ?', $label)
                ->where('t.bottle_type_id != ?', $entry_id);
    ...
    public function processForm($form_data) {
        ...
        if (!$this->checkLabel((int) $form_data['bottle_type_id'], $form_data['bottle_type'])) {
            throw new Exception("Duplicate bottle type not allowed.");
        }

        if ((int) $form_data['bottle_type_id'] > 0) {
            try {
                $bsize = $this->find($form_data['bottle_type_id']);
            ...
            $bsize = new Atlas_Model_BottleTypes();
            $bsize->setOptions($form_data);
            $this->save($bsize);
