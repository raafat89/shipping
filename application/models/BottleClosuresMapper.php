...
class Atlas_Model_BottleClosuresMapper {
    ...
    public function getDbTable() {
        ...
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Atlas_Model_DbTable_BottleClosures');
        }
    ...
    public function save(Atlas_Model_BottleClosures $b_closure) {
        // push the data into an array
        $data = $b_closure->toArray();

        // if the row in the db doesnt exist create the row
        // otherwise update the existing row
        if (NULL === ($bottle_closure_id = $b_closure->getBottle_closure_id()) || $bottle_closure_id == 0) {
            unset($data['bottle_closure_id']);
            $bottle_closure_id = $this->getDbTable()->insert($data);
            return $bottle_closure_id;
        } else {
            $this->getDbTable()->update($data, array('bottle_closure_id = ?' => $bottle_closure_id));
            return $bottle_closure_id;
        }
    }
    ...
    public function remove($bottle_closure_id) {
        $this->getDbTable()->delete("bottle_closure_id='$bottle_closure_id'");
    }

#end remove() function
    // find a row in the database based on the primary key and set the values
    // in the db object given by the user
    public function find($bottle_closure_id) {
        $b_closure = new Atlas_Model_BottleClosures();

        // attempt to locate the row in the database
        // if it doesn't exist throw an exception
        $result = $this->getDbTable()->find($bottle_closure_id);
        if (0 == count($result)) {
        ...
        $row = $result->current();
        $b_closure->setOptions($row->toArray());
    ...
    public function fetchAll() {
        ...
        foreach ($resultSet as $row) {
            $entry = new Atlas_Model_BottleClosures();
    ...
    public function selectAll() {
        ...
        $select->setIntegrityCheck(false)
                ->from(array("bc" => "bottle_closures"), array("bc.bottle_closure_id", "bc.bottle_closure"))
                ->order("bc.bottle_closure ASC");
    ...
    public function buildIdByName($name) {
        $select = $this->getDbTable()->select();
        $select->setIntegrityCheck(false)
                ->from(array("bc" => "bottle_closures"), array("bc.bottle_closure_id"))
                ->where("bc.bottle_closure LIKE '%" . $name . "%'");

        $result = $select->query()->fetchAll();
        return $result[0]['bottle_closure_id'];
    }
    ...
    public function checkLabel($entry_id, $label) {
        ...
        $select = $this->getDbTable()->select();
        $select->from(array("t" => "bottle_closures"), array('COUNT(*) AS count'))
                ->where('LOWER(t.bottle_closure) = ?', $label)
                ->where('t.bottle_closure_id != ?', $entry_id);
    ...
    public function processForm($form_data) {
        ...
        if (!$this->checkLabel((int) $form_data['bottle_closure_id'], $form_data['bottle_closure'])) {
            throw new Exception("Duplicate bottle closure not allowed.");
        }

        if ((int) $form_data['bottle_closure_id'] > 0) {
            try {
                $bsize = $this->find($form_data['bottle_closure_id']);
            ...
            $bsize = new Atlas_Model_BottleClosures();
            $bsize->setOptions($form_data);
            $this->save($bsize);
